...
LINE_HSV = (18, 255, 255)    # target color of the line (H, S, V)
...
def find_line(img):
    ...
    # hue is a circle (0 and 179 are neighbours): a range that runs past
    # one end continues from the other end
    h, t = LINE_HSV[0], LINE_TOL[0]
    if h - t < 0:
        mask |= cv2.inRange(hsv, (180 + h - t, int(lo[1]), int(lo[2])),
                            (179, int(hi[1]), int(hi[2])))
    elif h + t > 179:
        mask |= cv2.inRange(hsv, (0, int(lo[1]), int(lo[2])),
                            (h + t - 180, int(hi[1]), int(hi[2])))
