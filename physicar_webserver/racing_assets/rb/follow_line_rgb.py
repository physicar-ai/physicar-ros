...
# The rule: pixels near this RGB color ARE the line (each channel may be
# off by its tolerance). Everything else is road.
LINE_RGB = (255, 255, 0)     # target color of the line (R, G, B)
LINE_TOL = (60, 60, 60)      # allowed distance per channel (R, G, B)
...
CROP_SIDES = 0.0             # ignore this fraction on each side (clutter)
...
def find_line(img):
    ...
    lo = np.clip(np.array(LINE_RGB) - LINE_TOL, 0, 255)
    hi = np.clip(np.array(LINE_RGB) + LINE_TOL, 0, 255)
    mask = cv2.inRange(rgb, lo, hi)
    mask[:int(img.shape[0] * CROP_TOP)] = 0   # keep only the near road
    side = int(img.shape[1] * CROP_SIDES)
    if side:                                   # ...and away from the edges
        mask[:, :side] = 0
        mask[:, img.shape[1] - side:] = 0
    m = cv2.moments(mask)
...
<style>
  ...
  #sideL, #sideR { position: absolute; top: 0; bottom: 0; width: 0; pointer-events: none;
                   background: rgba(255,255,255,.06); }
  #sideL { left: 0; border-right: 1.5px dashed rgba(255,255,255,.55); }
  #sideR { right: 0; border-left: 1.5px dashed rgba(255,255,255,.55); }
...
<body>
  <img id="mask" alt="">
  <i id="crop"></i><i id="sideL"></i><i id="sideR"></i><i id="marker"></i>
  <canvas id="cv"></canvas>
...
(function () {
  ...
  es.onmessage = function (ev) {
    ...
    if (m.crop_sides !== undefined) { $("sideL").style.width = $("sideR").style.width = m.crop_sides * 100 + "%"; }
    ...
    while True:
        ...
        try:
            ...
            if m != settings_mtime:
                ...
                CROP_SIDES = float(cfg.get("crop_sides", CROP_SIDES))
                LINE_RGB = (int(cfg.get("red", LINE_RGB[0])),
                            int(cfg.get("green", LINE_RGB[1])),
                            int(cfg.get("blue", LINE_RGB[2])))
                LINE_TOL = (int(cfg.get("red_tol", LINE_TOL[0])),
                            int(cfg.get("green_tol", LINE_TOL[1])),
                            int(cfg.get("blue_tol", LINE_TOL[2])))
        ...
        # feed the live view (every 3rd frame: the mask drawn over the
        # dimmed camera image, so it is clear what the rule picked)
        ...
        if frame % 3 == 0:
            view = cv2.addWeighted(cv2.cvtColor(mask, cv2.COLOR_GRAY2BGR), 0.7,
                                   img, 0.4, 0)
            report["mask"] = base64.b64encode(
                cv2.imencode(".jpg", view)[1]).decode()
        report_view(**report)
