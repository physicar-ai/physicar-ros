...
from physicar_webserver.routers import racing
...
app.include_router(racing.router)     # /racing/api (course runner, no UI)
...
if _is_sim_mode():
    # Loopback runner for evaluation runs: the simulator's evaluation session (in its
    # own container) starts the student's code here so it always executes in the workspace.
    # nginx exposes /evaluation/ only to the simulator container — never the browser.
    from physicar_webserver.routers import evaluation as _evaluation
    app.include_router(_evaluation.router)
...
# real and sim; only the explicit page/API locations are proxied to this app.
