...
class AudioManager:
    ...
    def play(self, *, url: Optional[str] = None, path: Optional[str] = None,
        ...
        if self._is_sim():
            # kind/source ride along for the simulator's followers of this stream (the
            # evaluation observation and the video recorder) — the url alone does not
            # say which file a local item is.
            self._broadcast({
    ...
    def stream_feed(self, item_id: str, frames: bytes):
        ...
        if self._is_sim():
            self._broadcast({
                ...
                "volume": item.volume, "source": item.source,
            })
