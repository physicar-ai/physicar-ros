...
_JOBS = {
    # one script per colour space (rb/follow_line_hsv.py | _rgb.py) — the
    # student's code has no colour-space branching; see _job_script()
    "rb-run": {"script": "rb/follow_line_hsv.py", "home": "rb", "port5000": True},
...
_DEFAULTS = {
    # the line colour is matched in ONE colour space ("space") by one script
    # per space; both target sets are stored (the panel keeps them describing
    # the same colour) so the toggle never loses a tuned tolerance
    "rb": {"speed": 0.5, "gain": 20.0, "space": "hsv",
           "hue": 18, "sat": 255, "val": 255,
           "hue_tol": 8, "sat_tol": 90, "val_tol": 90,
           "red": 255, "green": 255, "blue": 0,
           "red_tol": 60, "green_tol": 60, "blue_tol": 60,
           "crop": 0.5, "crop_sides": 0.0, "pan": 0.0, "tilt": -15.0},
...
_LIMITS = {"speed": (0.1, 3.0), "gain": (0.0, 60.0), "left": (0.0, 20.0),
           ...
           "val_tol": (0, 255), "red": (0, 255), "green": (0, 255),
           "blue": (0, 255), "red_tol": (0, 255), "green_tol": (0, 255),
           "blue_tol": (0, 255), "crop": (0.0, 0.9), "crop_sides": (0.0, 0.45),
           "pan": (-30.0, 30.0), "tilt": (-30.0, 30.0)}
_MODES = ["greedy", "stochastic", "mean"]
_SPACES = ["hsv", "rgb"]


def _job_script(job, spec):
    """The script a job runs / shows: rb-run has one per colour space."""
    if job == "rb-run":
        return "rb/follow_line_%s.py" % _read_settings("rb")["space"]
    return spec["script"]
...
def settings_write(course: str, req: dict):
    ...
    prev = _read_settings(course)
    if req.get("reset"):
        cur = dict(_DEFAULTS[course])
    else:
        cur = _clamp(course, {**prev, **req})
    os.makedirs(_HOME, exist_ok=True)
    path = _settings_path(course)
    with open(path + ".tmp", "w") as f:
        json.dump(cur, f)
    os.replace(path + ".tmp", path)
    if course == "rb" and cur["space"] != prev["space"]:
        # the colour space is a different script, not a live knob: a running
        # rule-based drive restarts on the other one
        l = _lock_read()
        if l and l.get("job") == "rb-run":
            run_start(RunReq(job="rb-run"))
    return cur
...
def run_start(req: RunReq):
    ...
    script = os.path.join(_ASSETS, _job_script(req.job, spec))
...
def sample_code(job: str):
    ...
    path = os.path.join(_ASSETS, _job_script(job, spec))
    try:
        with open(path) as f:
            text = f.read()
    except OSError:
    ...
    if job == "rb-run":
        ...
        text = re.sub(r"(?m)^(CROP_SIDES = )[0-9.]+", r"\g<1>%s" % s["crop_sides"], text)
        if s["space"] == "rgb":
            text = re.sub(r"(?m)^(LINE_RGB = )\([0-9, ]+\)",
                          r"\g<1>(%d, %d, %d)" % (s["red"], s["green"], s["blue"]), text)
            text = re.sub(r"(?m)^(LINE_TOL = )\([0-9, ]+\)",
                          r"\g<1>(%d, %d, %d)" % (s["red_tol"], s["green_tol"], s["blue_tol"]),
                          text)
        else:
