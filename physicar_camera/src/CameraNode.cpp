...
private:
  ...
  mutable std::mutex undistort_mutex_;
  UndistortMaps undistort_maps_;
  bool undistort_enabled_ = false;

  void build_undistort_maps(int src_w, int src_h);
      ...
      )
{
  ...
  // undistortion parameters (0 = disabled)
  declare_parameter<double>("undistort.fx", 0.0);
  declare_parameter<double>("undistort.fy", 0.0);
  declare_parameter<double>("undistort.cx", 0.0);
  declare_parameter<double>("undistort.cy", 0.0);
  declare_parameter<double>("undistort.k1", 0.0);
  declare_parameter<double>("undistort.k2", 0.0);
  declare_parameter<double>("undistort.p1", 0.0);
  declare_parameter<double>("undistort.p2", 0.0);
  declare_parameter<double>("undistort.k3", 0.0);
  declare_parameter<double>("undistort.dist_scale", 1.0);
  declare_parameter<int64_t>("undistort.out_width", 0);
  declare_parameter<int64_t>("undistort.out_height", 0);
  ...
  while (true) {
    ...
    if (request->status() == libcamera::Request::RequestComplete) {
      ...
      if (format_type(cfg.pixelFormat) == FormatType::RAW) {
        ...
        // undistort (build maps on first frame or after param change)
        {
          const int src_w = static_cast<int>(cfg.size.width);
          const int src_h = static_cast<int>(cfg.size.height);
          bool need_rebuild = false;
          {
            std::lock_guard<std::mutex> lock(undistort_mutex_);
            need_rebuild = (undistort_maps_.src_w != src_w || undistort_maps_.src_h != src_h);
          }
          if (need_rebuild) {
            build_undistort_maps(src_w, src_h);
          }
        }

        if (undistort_enabled_) {
          try {
            auto cv_img = cv_bridge::toCvCopy(*msg_img);
            cv::Mat undistorted;
            {
              std::lock_guard<std::mutex> lock(undistort_mutex_);
              cv::remap(cv_img->image, undistorted, undistort_maps_.map1, undistort_maps_.map2, cv::INTER_LINEAR);
            }
            cv_img->image = undistorted;
            *msg_img = *cv_img->toImageMsg();
            msg_img->header = hdr;
          }
          catch (const cv_bridge::Exception &e) {
            RCLCPP_ERROR_STREAM(get_logger(), "undistort failed: " << e.what());
          }
        }
...
void
CameraNode::build_undistort_maps(int src_w, int src_h)
{
  const double fx = get_parameter("undistort.fx").as_double();
  const double fy = get_parameter("undistort.fy").as_double();
  const double cx = get_parameter("undistort.cx").as_double();
  const double cy = get_parameter("undistort.cy").as_double();
  const double scale = get_parameter("undistort.dist_scale").as_double();
  const double k1 = get_parameter("undistort.k1").as_double() * scale;
  const double k2 = get_parameter("undistort.k2").as_double() * scale;
  const double p1 = get_parameter("undistort.p1").as_double() * scale;
  const double p2 = get_parameter("undistort.p2").as_double() * scale;
  const double k3 = get_parameter("undistort.k3").as_double() * scale;
  const int out_w = static_cast<int>(get_parameter("undistort.out_width").as_int());
  const int out_h = static_cast<int>(get_parameter("undistort.out_height").as_int());
  ...
  const int dst_w = (out_w > 0) ? out_w : src_w;
  const int dst_h = (out_h > 0) ? out_h : src_h;

  cv::Mat new_camera_matrix = cv::getOptimalNewCameraMatrix(
    camera_matrix, dist_coeffs, cv::Size(src_w, src_h), 0.0, cv::Size(dst_w, dst_h));

  // Preserve aspect ratio: unify to the larger of fx/fy
  const double f_safe = std::max(new_camera_matrix.at<double>(0, 0), new_camera_matrix.at<double>(1, 1));
  new_camera_matrix.at<double>(0, 0) = f_safe;
  new_camera_matrix.at<double>(1, 1) = f_safe;

  std::lock_guard<std::mutex> lock(undistort_mutex_);
  cv::initUndistortRectifyMap(
    camera_matrix, dist_coeffs, cv::Mat(), new_camera_matrix,
    cv::Size(dst_w, dst_h), CV_16SC2, undistort_maps_.map1, undistort_maps_.map2);
  undistort_maps_.src_w = src_w;
  undistort_maps_.src_h = src_h;
  undistort_maps_.out_w = dst_w;
  undistort_maps_.out_h = dst_h;
  undistort_enabled_ = true;

  RCLCPP_INFO_STREAM(get_logger(),
    "undistort maps built: " << src_w << "x" << src_h
    << " -> " << dst_w << "x" << dst_h
    << ", k1=" << k1 << " scale=" << scale);
}
...
CameraNode::postParameterChange(const std::vector<rclcpp::Parameter> &parameters)
{
  ...
  for (const rclcpp::Parameter &parameter : parameters) {
    ...
    else if (parameter.get_name().rfind("undistort.", 0) == 0) {
      // Invalidate maps when undistort parameters change (rebuild on next frame)
      std::lock_guard<std::mutex> lock(undistort_mutex_);
      undistort_maps_.src_w = 0;
    }
  }
}
