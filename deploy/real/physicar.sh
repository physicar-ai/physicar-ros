...
(
  ...
  if $_ext_online; then
    ...
    # Major line = the robot software's own major (updater.sh follows v<major>.* tags
    # the same way): a v1.x robot installs physicar-ext 1.x only.
    _ext_major=$(git -C "$PHYSICAR_ROS_DIR" describe --tags --abbrev=0 HEAD 2>/dev/null | sed -n 's/^v\([0-9]\+\)\..*/\1/p')
    _ext_major="${_ext_major:-1}"
...
print(".".join(map(str, max(vs))) if vs else "")' "$_ext_major" 2>/dev/null)
    if [ -n "$_ext_want" ]; then
      ...
      _ext_out=$(timeout 60 /usr/local/bin/code-server --install-extension "physicar.physicar-ext@$_ext_want" --force 2>&1) || true
      echo "$_ext_out" | grep -i "successfully installed\|already installed\|Error\|Failed" | head -3 | sed 's/^/[physicar] ext: /'
    else
      echo "[physicar] ext: no physicar-ext ${_ext_major}.x on open-vsx - keeping $_ext_cur"
    fi
  ...
  # Guard extension built-in (deploy/ext-guard): re-enables physicar-ext
  # in-session when a student clicks Disable. Repo-direct through this refresh:
  # the updater moves the checkout, and the copy is a no-op until the files
  # differ or the folder is missing (a code-server update wipes lib/vscode).
  _guard_src="$PHYSICAR_ROS_DIR/deploy/ext-guard"
  _guard_dst="${CS_VSCODE:+$CS_VSCODE/extensions/physicar-ext-guard}"
  if [ -n "$_guard_dst" ] && [ -f "$_guard_src/package.json" ] \
     && ! { cmp -s "$_guard_src/package.json" "$_guard_dst/package.json" \
            && cmp -s "$_guard_src/extension.js" "$_guard_dst/extension.js"; } 2>/dev/null; then
    sudo rm -rf "$_guard_dst" 2>/dev/null
    if sudo mkdir -p "$_guard_dst" 2>/dev/null \
       && sudo cp "$_guard_src/package.json" "$_guard_src/extension.js" "$_guard_dst/" 2>/dev/null; then
      echo "[physicar] physicar-ext-guard built-in refreshed"
    fi
  fi
) &
