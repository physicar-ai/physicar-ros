...
if [ ! -f "$EXT_MARKER" ]; then
  (
    ...
    # Browser extension is bundled in the repo (no network fetch).
    VSIX_LOCAL="$PHYSICAR_ROS_DIR/deploy/device/physicar-browser-ext.vsix"
    if [ -f "$VSIX_LOCAL" ]; then
      /usr/local/bin/code-server --install-extension "$VSIX_LOCAL" &>/dev/null || true
    fi
