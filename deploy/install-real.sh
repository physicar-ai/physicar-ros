...
# ── Deep learning stack (identical to install-sim.sh — digital twin:
# student code must run unchanged on the robot). CPU-only torch wheel.
...
# Guard extension (deploy/ext-guard) as a built-in — re-enables physicar-ext
# in-session when a student clicks Disable (built-ins cannot be uninstalled, and
# VS Code has no "cannot disable" flag). Image-only, never published. Same vscode
# dir lookup as physicar.sh, which refreshes this copy at boot (updater-delivered).
...
if [ -n "$CS_VSCODE" ]; then
  ...
  cp "$PHYSICAR_ROS_DIR/deploy/ext-guard/package.json" "$PHYSICAR_ROS_DIR/deploy/ext-guard/extension.js" \
     "$CS_VSCODE/extensions/physicar-ext-guard/"
  echo "[ext-guard] staged as code-server built-in"
else
  echo "[ext-guard] WARNING: built-in staging skipped (vscode dir missing)"
fi
...
# inside physicar-ext, so nothing is seeded into the workspace any more
# (2026-09-06). Boot/update flows never write into the student
