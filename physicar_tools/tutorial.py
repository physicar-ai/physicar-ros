...
def status():
    ...
    accuracy-per-epoch curve. Call this after run() to watch progress and
    after a failure to read the reason.
    """
    m = _snapshot()
    out = {
        "job": m.get("job"),
...
def run(
    job: Annotated[str, Field(description="Which course script to start: 'sl-train' (train the supervised model), 'sl-run' (drive with the trained model), 'rb-run' (rule-based line following).")],
):
    ...
    """
    r = requests.post(f"{_API}/run", json={"job": job}, timeout=10)
    r.raise_for_status()
    tool_call_output_contents = [
...
def settings(
    course: Annotated[str, Field(description="Which course: 'sl' (Supervised Learning) or 'rb' (Rule-Based).")],
    values: Annotated[str, Field(description="JSON object of settings to change, e.g. '{\"speed\": 0.8}' or '{\"hue\": 100, \"hue_tol\": 12}'. Pass '{\"reset\": true}' for defaults. Omit to just read. sl keys: speed, left, right, pan, tilt, mode(greedy|stochastic|mean). rb keys: speed, gain, hue, sat, val, hue_tol, sat_tol, val_tol, crop, pan, tilt.")] = None,
):
