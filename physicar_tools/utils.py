...
def music_search(
    ...
    URL (play it with robot_audio_play) and view_url — the track's page on the
    iTunes Store / Apple Music.

    Attribution is required by the iTunes Search API terms whenever a result or
    a preview is used: when you play or list a track, tell the user it is a
    30-second preview provided courtesy of iTunes and give its view_url. Never
    present a preview as the full song.
    """
    r = requests.get(
    ...
    tool_call_output_contents = [
        {"type": "text", "text": json.dumps({
            "success": True,
            "attribution": "30-second previews provided courtesy of iTunes — say so and give the "
                           "view_url whenever you play or list a track.",
